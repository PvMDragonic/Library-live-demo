...
import { Tags as TagDatabase } from "../../database/tags";
...
export function EditTagEntry({ tag, index, activePickerRef, setTags }: IEditTagEntry)
{
    ...
    async function saveTag()
    {
        ...
        if (textInputRef.current?.value.trim() != '')
        {
            if (tag.id === -1)
            {
                ...
                await TagDatabase.create({ ...tag, label: trimmedLabel });
                const newTag = (await TagDatabase.searchByLabel(trimmedLabel) as Tags[])[0];
    ...
    {
        if (tag.id != -1)
            await TagDatabase.delete(tag.id);
